...
function [raw_dataw raw_data binned_data indexs indexinf index_deg  index_dist]= tau_vs_L(xx,L,ii,C,shift,mn)
 jk=[];
for ll=1;
    
node_all=find(xx(:,2)==0);
node_used=mod(node_all,length(L));
% L_node_used=L(node_used,1:end);
...
index_dist=find(y_dist<=12 & y_dissip>10^-12);
%index_dist=find(y_dist<=8 & x_L_4<500);
length(index_dist)
